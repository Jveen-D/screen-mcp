...
export const chartPanelCapability: JsonObject = {
  ...
  slots: {
    background: {
      ...
      multiple: false,
      description:
        "背景图，必须铺满整个模块。AI 必须主动设计并生成轻量 base64 科技风背景图；ChartPanel 不再提供默认背景，缺失 slots.background 时模块将没有背景组件。",
    },
    title: {
  ...
  layoutRules: [
    ...
    "background 必须由 AI 主动设计：优先使用 AI 生成的 imageBase64 科技风背景；无素材时也应生成 base64 渐变/网格/光效背景。MCP 不再提供默认背景，缺失 slots.background 时模块将没有背景组件。",
    ...
    "禁止在无用户禁止的情况下，让 LLM 把 background 留空或仅使用 style.backgroundColor 填充；每个模块都应由 LLM 提供经过设计的 SingleImage 背景或明显的 SVG 结构装饰。",
