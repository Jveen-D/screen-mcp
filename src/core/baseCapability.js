...
function hasPath(items, path) {
    return Array.isArray(items)
        ? items.some((item) => isJsonObject(item) && item.path === path)
        : false;
}
function withoutPath(items, path) {
    return Array.isArray(items)
        ? items.filter((item) => !isJsonObject(item) || item.path !== path)
        : [];
}
function appendUniqueByPath(items, additions) {
    const baseItems = Array.isArray(items) ? [...items] : [];
    const existingPaths = new Set(baseItems
        .filter(isJsonObject)
        .map((item) => item.path)
...
function ensureStyleChildren(items) {
    const baseItems = Array.isArray(items) ? [...items] : [];
    const styleItem = baseItems.find((item) => isJsonObject(item) && item.path === "style");
...
function baseStyleCapability() {
    return {
        path: "style",
        type: "object",
        description: "组件基础位置尺寸配置，对应 ChartPositionSetter。所有组件都必须包含 left、top、width、height、position。",
        children: [
            { path: "style.left", type: "number", description: "画布左侧距离。" },
            { path: "style.top", type: "number", description: "画布顶部距离。" },
            { path: "style.width", type: "number", description: "组件宽度。" },
...
function baseWritableProps(definition) {
    const backgroundPath = definition.componentType === "chart"
        ? "option.backgroundColor"
        : "style.backgroundColor";
    const backgroundDescription = definition.componentType === "chart"
    ...
    return [
        {
            path: "style",
            type: "object",
            description: "组件基础位置尺寸配置，对应 ChartPositionSetter；包含 left、top、width、height、position。style.zIndex 可保留默认值，层级由 ComponentSchema[] 输出顺序控制。",
        },
        {
            path: "rotate",
            type: "number",
        ...
        },
        {
            ...
            description: "组件入场动画配置，对应入场动画折叠面板。无明确动画要求时保持 isShow=false、type=''。",
            children: [
                {
                ...
                },
                {
                    ...
                    defaultValue: "",
                    description: "动画样式。只有 entryAnimiation.isShow=true 时才选择具体动画。",
                    options: [
...
function chartBaseWritableProps() {
    return [
        {
            path: "option.color",
            type: "array<string>",
        ...
        },
        {
            ...
            description: "常量数据行。AI 只允许填写 data 数组，每行使用 name、value，可选 type；MCP 会补齐 originalData、fieldList、dimension、indicator、sourceType 等完整 chartData 结构。",
            itemShape: {
                name: "分类名称，对应饼图扇区名称。",
        ...
        },
        {
            path: "option.legend",
            type: "object",
            description: "图例配置。legend.left 与 legend.top 必须成对选择合法位置，分别保存为字符串。",
            positionRules: {
                fields: ["left", "top"],
                description: "每一项的第一个值写入 legend.left，第二个值写入 legend.top，表示图例在容器的八个方位。",
                options: [
                    ["left", "top"],
                    ["center", "top"],
                    ["right", "top"],
            ...
            children: [
                ...
                },
                {
                    ...
                    description: "图例水平位置，必须与 option.legend.top 组合成 positionRules.options 中的一项。",
                },
                {
                    path: "option.legend.top",
                    type: "enum",
                    values: ["top", "center", "bottom"],
                    description: "图例垂直位置，必须与 option.legend.left 组合成 positionRules.options 中的一项。",
                },
                {
                    path: "option.legend.offsetX",
                    type: "number",
                    description: "图例水平偏移，单位 px。正数向右，负数向左。用于在保持 legend.left/legend.top 语义位置的基础上微调图例，不要用它替代正确的图例方位。",
                },
                {
                    path: "option.legend.offsetY",
                    type: "number",
                    description: "图例垂直偏移，单位 px。正数向下，负数向上。底部 legend 与外部 label 或底部装饰挤压时，通常使用 -4 到 -14 让 legend 轻微上移。",
                },
                {
...
export function withBaseCapability(definition) {
    ...
    capability.layerRules = {
        description: "渲染层级由 ComponentSchema[] 输出顺序控制：数组越靠前越在顶层，数组越靠后越在底层。AI 不需要通过 style.zIndex 控制层级。",
    };
    capability.baseConfig = {
        description: "所有组件共享基础配置：位置尺寸、旋转角度、不透明度和背景颜色。渲染层级由 ComponentSchema[] 输出顺序控制。",
        setters: [
            {
                path: "style",
                setter: "ChartPositionSetter",
                description: "位置尺寸配置。style 必须包含 left、top、width、height、position；zIndex 仅作为兼容字段保留默认值。",
            },
            {
                path: "rotate",
                setter: "NumberSetter",
            ...
            {
                path: definition.componentType === "chart"
                    ? "option.backgroundColor"
                    : "style.backgroundColor",
                setter: "ColorSetter",
                description: definition.componentType === "chart"
            ...
            },
            {
                path: "entryAnimiation",
                setter: "CollapsePanel",
                description: "入场动画配置。entryAnimiation.isShow 使用 SwitchSetter，entryAnimiation.type 使用 SelectSetter；无明确要求时保持关闭。",
                defaultValue: {
                    isShow: false,
                    type: "",
                },
    ...
    capability.aiWritableProps = appendUniqueByPath(capability.aiWritableProps, baseWritableProps(definition));
    if (definition.componentType === "chart") {
        capability.aiWritableProps = appendUniqueByPath(capability.aiWritableProps, chartBaseWritableProps());
    }
    return capability;
}
