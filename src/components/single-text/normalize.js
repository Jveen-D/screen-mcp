...
function normalizeLineHeight(style) {
    ...
    const normalized = typeof fontSize === "number" && fontSize > 0 ? lineHeight / fontSize : 1.35;
    style.lineHeight = Math.min(Math.max(Number(normalized.toFixed(2)), 1), 2);
}
export function normalizeSingleTextProps(props) {
    props.textOverflow = normalizeEnum(props.textOverflow, ["ellipsis", "clip", "visible"], "ellipsis");
    props.verticalAlign = normalizeEnum(props.verticalAlign, ["top", "center", "bottom"], "center");
