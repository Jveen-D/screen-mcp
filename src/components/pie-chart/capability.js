export const pieChartCapability = {
    componentName: "PieChart",
    displayName: "饼图",
    description: "用于展示分类占比、构成比例和环形占比关系的 ECharts 饼图组件。",
    aiRole: "AI 负责生成组件布局、视觉表达和可选的 chartData.constant.data 语义数据；MCP 会补齐完整 props 与有效 chartData。组件层级由最终 schema 数组顺序决定。",
    requiredProps: [
        {
            path: "componentName",
            type: "string",
    ...
    aiWritableProps: [
        ...
        },
        {
            ...
            type: "object",
            description: "饼图唯一系列配置。AI 不需要生成 type、data 或 dataset，MCP 会按下标深合并默认配置。",
            children: [
                {
                    path: "option.series[0].radius",
                    type: "[string,string]",
                    description: "内外半径，格式为 [innerRadius, outerRadius]，通常使用百分比字符串。innerRadius 为 '0%' 时是实心饼图，非 0 时是环形图；外部 label、底部 legend 或侧边摘要拥挤时，应适当减小 outerRadius，例如 ['36%', '54%']。",
                },
                {
                    path: "option.series[0].center",
                    type: "[string,string]",
                    description: "圆心位置，格式为 [x, y]，通常使用相对图表容器的百分比字符串，例如 ['50%', '50%']。底部 legend 与外部 label 挤压时，可把 y 调整到 '42%' 到 '46%'，让饼图主体上移。",
                },
                {
                ...
                },
                {
                    ...
                    formatterRules: {
                        ...
                        tokens: [
                            ...
                            {
                                token: "{@xxx}",
                                meaning: "数据中名为 xxx 的维度的值，例如 {@product} 表示名为 product 的维度值。",
                            },
                            {
                                token: "{@[n]}",
                                meaning: "数据中第 n 个维度的值，例如 {@[3]} 表示维度 3 的值，从 0 开始计数。",
                            },
                            {
                                token: "\\n",
                                meaning: "换行符，用于把标签拆成多行显示，例如 {b}\\n{c}。",
                            },
                        ],
                        examples: ["{c}", "{b}: {c}", "{b}\\n{c}", "{b}: {@value}"],
                    },
    ...
    aiForbiddenProps: [
        ...
        },
        {
            ...
            reason: "PieChart 的 series type 固定为 'pie'，MCP 会强制回写，AI 不应覆盖为其他图表类型。",
        },
        {
        ...
        {
            path: "option.dataset",
            reason: "当前渲染链路不使用 ECharts dataset 驱动 PieChart 数据，AI 写入 dataset 会被忽略并造成误导。",
        },
        {
            path: "option.title",
