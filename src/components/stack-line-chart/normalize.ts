...
function normalizeDimension(chartData: JsonObject): void {
  const dimension = chartData.dimension;
  const dims: JsonObject[] = Array.isArray(dimension)
    ? dimension.filter(isJsonObject)
    : [];

  const hasName = dims.some((item) => item.fieldName === "name");
  const hasType = dims.some((item) => item.fieldName === "type");
  ...
  chartData.dimension = dims;
}

function normalizeShowSymbol(item: JsonObject): void {
...
export function normalizeStackLineChartProps(props: JsonObject): JsonObject {
  normalizeStackLineChartData(props);

  const chartData = props.chartData;
  if (isJsonObject(chartData)) {
    normalizeDimension(chartData);
  }
  ...
  normalizeStackLineSeries(option);
