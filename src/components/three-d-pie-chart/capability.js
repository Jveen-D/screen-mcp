export const threeDPieChartCapability = {
    componentName: "ThreeDPieChart",
    displayName: "3D饼图",
    description: "基于 Three.js 的 3D 饼图组件，支持立体厚度、俯视角度、扇区抬升、轮播动画和中心标签，用于展示分类占比和构成比例。",
    aiRole: "AI 负责生成组件布局、视觉表达和可选的 chartData.constant.data 语义数据；MCP 会补齐完整 props 与有效 chartData。3D 饼图的 label 默认关闭，信息展示依赖中心标签和侧边摘要。组件层级由最终 schema 数组顺序决定。",
    requiredProps: [
        {
            path: "componentName",
            type: "string",
        ...
        },
        {
            path: "logicalId",
            type: "string",
            description: "组件唯一 ID，由 AI 生成，用于编辑器大纲树和 schema businessElementId；后端限制最长 50 个字符，必须包含短随机段以保证大屏内全局唯一。",
        },
        {
    ...
    aiWritableProps: [
        ...
        },
        {
            ...
            type: "object",
            description: "3D 饼图唯一系列配置。AI 不需要生成 type、data 或 dataset，MCP 会按下标深合并默认配置。",
            children: [
                {
                    path: "option.series[0].radius",
                    type: "[string,string]",
                    description: "内外半径，格式为 [innerRadius, outerRadius]，通常使用百分比字符串。3D 饼图默认半径较大（如 ['72%', '96%']），组件较小时应适当减小 outerRadius，例如 ['56%', '78%']。",
                },
                {
                    path: "option.series[0].center",
                    type: "[string,string]",
                    description: "圆心位置，格式为 [x, y]，通常使用相对图表容器的百分比字符串，例如 ['50%', '48%']。底部 legend 挤压时，可把 y 调整到 '42%' 到 '46%'，让饼图主体上移。",
                },
                {
        ...
        },
        {
            ...
            type: "object",
            description: "3D 视图专属配置，控制立体厚度、视角、动画等效果。AI 只应修改以下子项，其余由 MCP 保留默认值。",
            children: [
                {
                ...
                },
                {
                    ...
                    type: "number",
                    range: [0, 85],
                    description: "俯视角度，单位度。0 为完全平视（侧面），85 接近顶视；大屏常用 50–70 之间，既能体现立体感又保证可读性。",
                },
                {
                ...
                },
                {
                    ...
                    type: "boolean",
                    description: "是否显示中心标签（当前激活扇区的名称和占比）。3D 饼图的主要信息展示方式，默认 true。",
                },
                {
                    path: "option.threeDSettings.liftDistance",
                    type: "number",
                    range: [0, 160],
                    description: "轮播时扇区抬升高度，单位 px。数值越大高亮扇区抬升越明显；保守面板可使用 8–14，强调动效可使用 20–40。",
                },
                {
                    path: "option.threeDSettings.animationDuration",
                    type: "number",
                    range: [400, 12000],
                    description: "轮播动画间隔，单位毫秒。默认 2200ms；数据项较多或面板强调稳重感时可适当增大到 3000–4000ms。",
                },
                {
                    path: "option.threeDSettings.tilt",
                    type: "number",
                    range: [0, 90],
                    description: "倾斜角度，与俯视角度联动控制 3D 观感。通常保持与 topViewAngle 接近即可，大屏默认约 63°。",
                },
            ],
        },
    ],
    aiForbiddenProps: [
        ...
        },
        {
            ...
            reason: "ThreeDPieChart 的 series type 固定为 'pie'，MCP 会强制回写，AI 不应覆盖为其他图表类型。",
        },
        {
        ...
        {
            path: "option.dataset",
            reason: "当前渲染链路不使用 ECharts dataset 驱动 ThreeDPieChart 数据，AI 写入 dataset 会被忽略并造成误导。",
        },
        {
            path: "option.title",
    ...
    visualRules: [
        ...
        "色彩要服务主题：等级、状态类可以使用同一色系的明暗层级；类别、分类、品类类可以使用主色、辅色、强调色和低饱和补色组合。",
        ...
        "3D 饼图的阴影和发光效果应与面板整体风格统一：强视觉主题可用更明显的发光分割；简洁风应使用克制描边。",
    ],
    examples: [
        {
            title: "3D 饼图配置示例",
